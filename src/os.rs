...
impl Os {
    pub fn new() -> Self {
        let (mut pretty_name, mut logo, mut wm) =
        ...
        let os_release = fs::read_to_string(Path::new("/etc/os-release")).expect("Could not access `/etc/os-release`");
